...
func (r *Repository) ExtractCropsNameCalculate(name string) (entity.CalculateResponse, error) {
	var response entity.CalculateResponse

	log.Printf("Consulta SQL: %s", ExtractCropsNameCalculateQuery)
	log.Printf("Name del crop: %s", name)

	err := r.db.QueryRow(ExtractCropsNameCalculateQuery, name).
		Scan(
			&response.Name,
